...
export default function Tickets({ onNavigate }) {
  ...
  return (
    <main className="flex-1 flex flex-col h-full min-h-0 w-full bg-[#E8EEF5] text-slate-800 p-4 sm:p-5 lg:p-6 space-y-4 no-scrollbar max-w-7xl mx-auto overflow-hidden">
      ...
      {/* ─────────────────────────────────────────────────────────────────────────
          2. FILTER & SEARCH TOOLBAR (Neumorphic Inset / Raised Surfaces)
         ───────────────────────────────────────────────────────────────────────── */}
      <div className="shrink-0 flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-3">
        {/* Sunken Search Input */}
        <div className="relative flex-1 lg:max-w-md group/search">
          ...
          <input
            ...
            placeholder="Search by ID (#TKT-001), customer, topic, or keyword..."
            className="neu-input w-full pl-10 pr-14 py-2.5 text-xs font-bold text-slate-800 placeholder:text-slate-400 focus:outline-none"
          />
          <div className="absolute inset-y-0 right-0 pr-3 flex items-center gap-1 z-20">
            {searchTerm ? (
              ...
              <kbd className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-lg bg-[#E2E9F2] shadow-neu-inset text-[10px] text-slate-500 font-mono">
                ⌘K
              </kbd>
            )}
          </div>
        ...
        <div className="flex items-center gap-2.5 self-start lg:self-auto flex-wrap">
          ...
          {isFiltered && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#E8EEF5] shadow-neu-btn active:shadow-neu-btn-pressed border border-white/80 text-slate-600 hover:text-slate-900 rounded-2xl text-xs font-bold transition-all cursor-pointer"
            >
