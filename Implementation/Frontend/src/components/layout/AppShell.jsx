...
export default function AppShell({ currentPath, onNavigate, children }) {
  ...
  return (
    <div className="h-screen max-h-screen w-full bg-[#F7F9FC] flex flex-col md:flex-row text-slate-900 font-sans overflow-hidden relative">
      ...
      <header
        className={`md:hidden flex items-center justify-between px-4 py-3 bg-[#011662] text-white border-b border-[#011E79] shrink-0 z-30 transition-transform duration-300 ease-in-out ${isNavVisible ? 'translate-y-0' : '-translate-y-full pointer-events-none'
          }`}
      >
      ...
      <nav
        ...
        className={`md:hidden fixed bottom-0 inset-x-0 bg-[#011662]/95 backdrop-blur-md border-t border-[#011E79] text-white z-30 transition-all duration-300 ease-in-out px-2 py-1 shadow-2xl ${mobileSidebarOpen
      ...
      >
        <div className="flex items-center justify-around">
          <button
            type="button"
            onClick={() => onNavigate && onNavigate('/dashboard')}
            className={`flex flex-col items-center gap-1 py-1 px-2.5 rounded-lg text-[10px] font-medium transition-colors cursor-pointer ${currentPath === '/dashboard' || currentPath === '/'
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 py-1 px-2.5 rounded-lg text-[10px] font-medium transition-colors cursor-pointer ${currentPath && currentPath.startsWith('/tickets') && currentPath !== '/tickets/create'
                ? 'text-brand-cyan font-bold'
                : 'text-blue-200/80 hover:text-white'
              }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 py-1 px-2.5 rounded-lg text-[10px] font-medium transition-colors cursor-pointer ${currentPath && currentPath.startsWith('/customers')
                ? 'text-brand-cyan font-bold'
                : 'text-blue-200/80 hover:text-white'
              }`}
          >
          ...
          <button
            ...
            className={`flex flex-col items-center gap-1 py-1 px-2.5 rounded-lg text-[10px] font-medium transition-colors cursor-pointer ${currentPath && currentPath.startsWith('/ai')
                ? 'text-brand-cyan font-bold'
                : 'text-blue-200/80 hover:text-white'
              }`}
          >
